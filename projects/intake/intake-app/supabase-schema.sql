-- =============================================================
-- Intake App Database Schema
-- Runs in the SAME Supabase project as Jarvis
-- Tables prefixed with intake_ to avoid conflicts
-- Jarvis uses service_key so he bypasses RLS and sees everything
-- =============================================================
...
-- Jarvis uses service_key, bypasses RLS entirely
-- =============================================================
...
-- These views let Jarvis monitor Intake as a business
-- Queried via service_key (bypasses RLS)
-- =============================================================
...
SELECT
  ...
  -- Revenue estimate (Plus=$4.99, Pro=$9.99 monthly)
  (SELECT
    COALESCE(SUM(CASE WHEN subscription_tier = 'plus' THEN 4.99 WHEN subscription_tier = 'pro' THEN 9.99 ELSE 0 END), 0)
  FROM intake_profiles) AS monthly_revenue_estimate;

-- User retention: daily active users over last 30 days
...
-- Feature usage breakdown (which features drive engagement)
...
-- Conversion funnel: signup → first log → 7-day retention → paid
...
SELECT
  ...
  (SELECT COUNT(DISTINCT fe.user_id)
   FROM intake_food_entries fe
   JOIN intake_profiles p ON fe.user_id = p.id
   WHERE fe.logged_at > p.created_at + INTERVAL '7 days') AS retained_7_days,
